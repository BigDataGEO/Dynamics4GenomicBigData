...
for i=1:length(unique_GEO_numbers)
  ...
  fprintf('\n');
  display(['All results from dataset ' unique_GEO_numbers{i} ' have been output to folder ' Dynamics4GenomicBigData_HOME 'Results/' unique_GEO_numbers{i} '/Conditions/']);

  write_study_report(unique_GEO_numbers{i});

  fprintf('\n');
  display(['A consolidated report on all conditions from dataset ' unique_GEO_numbers{i} ' can be found in ' Dynamics4GenomicBigData_HOME 'Results/' unique_GEO_numbers{i} '/paper.pdf']);
